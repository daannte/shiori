...
export const load: PageLoad = async ({ fetch, params }) => {
  ...
  let media = await loadMedia(client, mediaId)
...
async function loadMedia(client: ReturnType<typeof createClient>, id: number): Promise<MediaResponse> {
  let res = await client.GET("/api/v1/media/{id}", { params: { path: { id } } }).catch(() => {
    loadMediaError(504)
  })
